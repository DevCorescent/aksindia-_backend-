...
const PAYMENT_METHODS = ['card', 'upi', 'wallet', 'cod'] as const;
...
export const ordersService = {
  async list(role: UserRole, userId?: string, storeId?: string): Promise<Order[]> {
    ...
    if (role === 'customer' && userId) {
      where.push(`customer_id = $${params.push(userId)}`);
    }
    ...
    if (role === 'agent' && userId) {
      where.push(`agent_id = $${params.push(userId)}`);
    }
  ...
  async create(payload: Omit<Order, 'id'>): Promise<Order> {
    if (!PAYMENT_METHODS.includes(payload.paymentMethod as any)) {
      throw new Error('Invalid payment method');
    }

    if (!PAYMENT_STATUSES.includes(payload.paymentStatus as any)) {
      throw new Error('Invalid payment status');
    }

    if (!ORDER_STATUSES.includes(payload.status as any)) {
    ...
    const storeId =
      payload.storeId && UUID_RE.test(payload.storeId)
        ? payload.storeId
        : null;
    ...
    const row = await queryOne(
      ...
      [
        ...
        payload.subtotal,
        payload.total,
        payload.commissionTotal,
        payload.adminRevenue,
        payload.discount ?? 0,
  ...
  async update(id: string, patch: Partial<Order>): Promise<Order> {
    if (
      ...
      !PAYMENT_METHODS.includes(patch.paymentMethod as any)
    ) {
      throw new Error('Invalid payment method');
    }
    ...
    if (
      ...
      !PAYMENT_STATUSES.includes(patch.paymentStatus as any)
    ) {
      throw new Error('Invalid payment status');
    }
    ...
    if (
      ...
      !ORDER_STATUSES.includes(patch.status as any)
    ) {
      throw new Error('Invalid order status');
    }
    ...
    if (patch.courierName !== undefined) {
      fields.push(`courier_name = $${i++}`);
      values.push(patch.courierName);
    }

    if (patch.cancelReason !== undefined) {
      fields.push(`cancel_reason = $${i++}`);
      values.push(patch.cancelReason);
    }
